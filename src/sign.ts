...
/** 当前毫秒时间戳（字符串）。 */
export function dingTalkTimestamp(now: number = Date.now()): string {
 ...
 */
export function computeDingTalkSign(secret: string, timestamp: string): string {
...
export function signedUrl(webhook: string, secret: string | undefined, timestamp?: string): string {
